...
def get_statictics(stock,adj):
    columns=['return','MA5','MA10','MA22','vol5','vol30','week_mmtm','month_mmtm']
    res=pd.DataFrame(None,index=adj.index,columns=pd.MultiIndex.from_product([stock,columns]))
    for st in stock:
        res[st,'return']=adj[stock]/adj[stock].shift(1)-1
        res[st,'MA5']=adj[stock].rolling(5).mean()
        res[st,'MA10']=adj[stock].rolling(10).mean()
        res[st,'MA22']=adj[stock].rolling(22).mean()
        res[st,'vol5']=adj[stock].rolling(5).std()
        res[st,'vol30']=adj[stock].rolling(30).std()
        res[st,'week_mmtm']=adj[stock]-adj[stock].shift(5)
        res[st,'month_mmtm']=adj[stock]-adj[stock].shift(22)
